...
const domPos = function (options) {
    ...
    // set baseOn Offse
    options.el.$overallWidth = options.el.clientWidth + getBorderWidth(options.el).left + getBorderWidth(options.el).right, 'el'
    positionLeft = getBaseOnOffset(
        positionLeft,
        options.baseOn.el[0],
        options.el.$overallWidth
    )
    options.el.$overallHeight = options.el.clientHeight  + getBorderWidth(options.el).top + getBorderWidth(options.el).bottom, 'el'
